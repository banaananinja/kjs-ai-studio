...
function App() {
  ...
  useEffect(() => { const newAncestorPaths = {}; for (const path in selectedFilePaths) { let current = getParentPath(path); while (current !== null) { newAncestorPaths[current] = true; current = getParentPath(current); } } setAncestorPaths(newAncestorPaths); console.log("Updated ancestor paths:", newAncestorPaths); }, [selectedFilePaths]);

  const combinedTokenCount = conversationTokenCount + processedFilesTokenCount;

  return (
    // *** Add conditional class to App div ***
    <div className={`App ${isRightSidebarExpanded ? 'right-sidebar-expanded' : ''}`}>
      ...
      <RightSidebar
         ...
         triggerFileProcessing={triggerFileProcessing}
         clearProcessedFiles={clearProcessedFiles}
         fileProcessingLoading={fileProcessingLoading}
         addDebugLog={addDebugLog}
         // *** Pass expansion state and toggle function down ***
         isExpanded={isRightSidebarExpanded}
         toggleExpansion={toggleRightSidebar}
       />
    </div>
  );
}
