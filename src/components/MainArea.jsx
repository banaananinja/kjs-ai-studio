...
const MainArea = forwardRef(({ /* ...props... */
  ...
  useEffect(() => { /* Token count effect */ const calculateConversationTokenCount = async () => { let conversationContent = ""; if (systemInstructions.trim()) conversationContent += `System: ${systemInstructions.trim()}\n\n`; conversationContent += messages.map(msg => `${msg.role === 'assistant' ? 'Assistant' : 'User'}: ${msg.content}`).join('\n'); try { const count = await countTokens(conversationContent, selectedModel); const limit = getTokenLimit(selectedModel); onTokenCountChange(count, limit); } catch (error) { console.error('Error counting conversation tokens:', error); const limit = getTokenLimit(selectedModel); onTokenCountChange(0, limit); } }; calculateConversationTokenCount(); }, [messages, systemInstructions, selectedModel, onTokenCountChange]);
  ...
  // Keep toggleDropdown simple - state sets which one is open
  const toggleDropdown = (messageId, event) => {
    event.stopPropagation();
    if (activeDropdown && activeDropdown.id === messageId) {
      setActiveDropdown(null);
    } else {
      setActiveDropdown({ id: messageId }); // Just store the ID
    }
  };
